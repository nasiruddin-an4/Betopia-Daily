...
export async function POST(request) {
  try {
    ...
    if (microsoft_access_token) {
      // New profile endpoint using microsoft_access_token
      erpRes = await fetch('https://erp.betopiagroup.com/grocery-api/profile', {
        method: 'POST',
        headers: {
      ...
      // Legacy profile endpoint using email and API Key
      erpRes = await fetch('https://erp.betopiagroup.com/grocery-api/profile/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    console.error('ERP Profile proxy error:', error);
    return NextResponse.json({ success: false, error: 'Failed to fetch from ERP profile API' }, { status: 500 });
  }
}
